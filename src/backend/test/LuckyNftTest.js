...
describe('LuckyNFT', () => {
	...
	describe('Deployment', () => {
		...
		it('initializes the contract with the correct values', async () => {
			...
			expect(await nft.tokenSupply()).to.equal(100)
	...
	describe('handles transfers', () => {
		...
		it('transfers ownership', async () => {
			await nft.connect(addr1)._mint(addr1.address)
			expect(await nft.connect(addr1).transferFrom(addr1.address ,addr2.address, 1)).to.emit(nft, "Transfer").withArgs(
				addr1.address,
				addr2.address,
				1
				)
